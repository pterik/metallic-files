...
type
  ...
  TDM = class(TDataModule)
    ...
    qHdrGlobExists: TZQuery;
    qHdrGlobInsert: TZQuery;
    qHdrExists: TZQuery;
    qHdrInsert: TZQuery;
    ...
    procedure RepaintGrid(var Grid:TDBGridEh; Treeid:integer;GridFields:GridParameters);
    //  ==========================================
...
//  Функции и процедуры имеют префикс- имя таблицы. Сортировка по имени функции
		...
		function  CompanyInsert(CompanyName, City:string; TrustLevel:integer; SComment:string):integer;
		function  CompanyUpdate(CompanyID:integer; CompanyName, City:string; TrustLevel:integer; SComment: string): boolean;
...
function TDM.CompanyInsert(CompanyName, City:string; TrustLevel:integer; SComment:string): integer;
var Res:integer;
begin
try
Res:=CompanyMaxID+1;
...
function TDM.CompanyUpdate(CompanyID:integer; CompanyName, City:string; TrustLevel:integer; SComment: string): boolean;
begin
try
QUCompany.Close;
...
for I := 0 to Grid.Columns.Count - 1 do    // Iterate
  ...
  qHdrGlobExists.Open;
  if VarIsNull(qHdrGlobExists['CNTR'])
  then Cntr:=0
  else Cntr:=qHdrGlobExists['CNTR'];
  if Cntr=0 then
    begin
    ...
    qHdrGlobInsert.ParamByName('DISPLAYFORMAT').AsString:=Grid.Columns[i].DisplayFormat;
    ...
    qHdrGlobUpdate.ParamByName('DISPLAYFORMAT').AsString:=Grid.Columns[i].DisplayFormat;
...
for I := 0 to Grid.Columns.Count - 1 do    // Iterate
  ...
  qHdrExists.Open;
  if VarIsNull(qHdrExists['CNTR'])
  then Cntr:=0
  else Cntr:=qHdrExists['CNTR'];
  if Cntr=0 then
    begin
    qHdrInsert.Close;
...
while not QDisplay.Eof do
  begin
  if not (FCompany in GridFields) and (QDisplay['GS_FIELD']='CM_NAME')
    then
      begin
      QDisplay.Next;
      continue;
      end;
  if not (FCompany in GridFields) and (QDisplay['GS_FIELD']='CM_CITY')
    then
      begin
      QDisplay.Next;
      continue;
      end;
  if not (FTree in GridFields) and (QDisplay['GS_FIELD']='PL_PARENT')
    then
      begin
      QDisplay.Next;
      continue;
      end;
  if not (FTree in GridFields) and (QDisplay['GS_FIELD']='PT_VALUE')
    then
      begin
      QDisplay.Next;
      continue;
